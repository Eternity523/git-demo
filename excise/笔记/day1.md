...
路由传递参数先关面试题
     ...
     不可以：不能这样书写，程序会崩掉
     路由跳转传参的时候，对象的写法可以使name与path结合，但是path不能与params结合使用
     2:如何指定params参数可传可不传? 
     如果路由要求params参数可以传递或不传递,在路由占位的时候后面可以加上一个问号,(?与正则表达式的规则相同，表示出现一次或者0次)
     3:params参数可以传递也可以不传递，但是如果传递是空串，如何解决？
     使用undefined解决，parama参数可以传递、不会传递("空的字符串")
